<template lang="pug">
  .container(:class="type")
    .particle(v-for="particle in particles", :class="particle.color",
        :style="particle.style")
</template>

<script>
  export default {
    props: {
      type: {
        type: String,
        default: null
      },
      count: {
        type: Number,
        default: 0
      }
    },
    computed: {
      particles() {
        return this.type === 'confetti' && this.count ? _.times(this.count, () => ({
          color: 'c' + _.random(1, 2),
          style: {
            top: _.random(10, 50) + '%',
            left: _.random(1, 99) + '%',
            width: _.random(10, 18) + 'px',
            height: _.random(12, 20) + 'px',
            animationDelay: _.random(0, 1500) + 'ms'
          }
        })) : [];
      }
    }
  }

</script>

<style scoped lang="stylus" type="text/stylus">
  .container
    posit(absolute, 0);

    >.particle
      position: absolute;

  .confetti
    >.particle
      opacity: 0;
      animation: confetti 1s ease-in;

      &.c1
        background-color: rgba(76,175,80,0.5);

      &.c2
        background-color: rgba(156,39,176,0.5);

  @keyframes confetti
    0%
      opacity: 0;
      transform: translateY(0) rotate(0deg);
    10%
      opacity: 1;
    35%
      transform: translateY(-600%) rotate(270deg);
    80%
      opacity: 1;
    100%
      opacity: 0;
      transform: translateY(1800%) rotate(1440deg);

</style>
